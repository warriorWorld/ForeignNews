...
class Printer {
    ...
    static void printJsonRequest(LoggingInterceptor.Builder builder, Request request) {
        ...
        logLines(builder.getType(), tag,getRequestUrl(request), builder.getLogger(), false);
        logLines(builder.getType(), tag,getToken(request), builder.getLogger(), false);
    ...
    static String[] getToken(Request request){
        String token = "ssi-token: "+request.header("ssi-token");
    ...
    private static String[] getRequestUrl(Request request){
        if("GET".equals(request.method())){
            String text = URL_TAG + request.url();
            try {
                return new String[]{URLDecoder.decode(text.toString(), "utf-8")};
            } catch (UnsupportedEncodingException e) {
                return new String[]{text};
            }
        }else {
            return new String[]{URL_TAG + request.url()};
        }
